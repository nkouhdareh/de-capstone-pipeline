"""Streamlit dashboard (enhanced) - openFDA drug-safety signal screening.

Reads the dbt marts in DE_CAPSTONE.DBT_DEV with read-only SELECT queries.
It computes nothing: PRR / ROR / ROR-CI / chi2 all come from the dbt macros in
macros/signal_metrics.sql, so the project keeps exactly one definition of every
metric, from the warehouse to the screen.

app/dashboard.py is the original and is left untouched as the rollback version.

Run (the dark theme is a launch flag - no config file is created):

    .venv-app/Scripts/streamlit.exe run app/dashboard_enhanced.py \
...
@st.cache_resource
def get_connection():
    """One Snowflake session, key-pair auth via app/db.py."""
    return connect()
...
def load_volcano(min_cases):
    ...
    return run_query(
        ...
        " from sem_signal_metrics"
        " where a >= %s and prr is not null and chi2_yates is not null"
...
def load_heatmap_source(min_cases):
    ...
    return run_query(
        ...
        " from sem_signal_metrics"
        " where is_signal and prr is not null and a >= %s",
        (int(min_cases),),
    )
