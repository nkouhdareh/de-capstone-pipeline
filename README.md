...
![dbt lineage graph: two raw sources through staging and drug-name resolution into four dimensions and a fact table, then signal metrics and assertion tests](docs/assets/dbt-dag-refinement.png)

Generated by dbt from the model refs, not drawn by hand. Two raw sources pass through
staging and `int_drug_resolution`, which collapses the brand, generic and misspelled
name variants, into four dimensions and `fct_report_drug_reaction`. The disproportionality
metrics sit downstream, and the assertions are models in their own right:
`assert_signal_worked_example` is the clozapine acceptance check from [Results](#results),
...
![Live dashboard KPIs: 45,030,932 Silver rows, 1,240,645 scored pairs, 315,270 candidate signals, models last built 2026-08-15](docs/assets/dashboard_0_kpis.png)

The same figures live from the dashboard, with the build timestamp, so the numbers
below are not a claim about a run that happened once.
...
![Top 20 candidate signals by case count, clozapine to neutropenia leading with 5,571 cases and flagged as a strict signal](docs/assets/dashboard_1_signals.png)

The chart behind that claim. Clozapine to neutropenia sits at the top of the ranked
list, and the red bars are the pairs that clear the strict threshold.

---

## The dashboard

The Streamlit app runs **inside Snowflake**, so there is no public URL to hand out: it
is reachable only from an account with access to the warehouse. These screenshots are
the demo. The code is in [`app/dashboard.py`](app/dashboard.py), and the app issues
read-only SELECT queries against the marts, computing no metrics of its own.

[![Full dashboard overview showing the KPI strip, a pipeline funnel from 93.4M atomic rows down to 315,270 candidate signals, the share of scored pairs by flag, and a volcano plot of disproportionality against statistical support](docs/assets/dashboard_overview_full.png)](docs/assets/dashboard_overview_full.png)

The Overview tab end to end. The funnel carries 93.4M atomic rows down to 315,270
candidates, the donut shows how few of those clear the strict threshold, and the volcano
plot is the argument for triage rather than a binary flag: genuine signals and known
artifacts occupy the same top-right region, so no cut on that plane separates them.
Click the image for full size.

### The ranked candidates

![Ranked candidate table with columns for cases, PRR, ROR, lower bound of the ROR confidence interval, chi-square, and the signal and strict flags](docs/assets/dashboard_2_table.png)

Every candidate with the evidence beside it, so an analyst judges the pair instead of
trusting a flag. The top row is the clozapine result quoted above. The third row is the
point of the whole table: clozapine to death has 1,848 cases but a PRR of 1.92, so it is
not flagged. Volume alone is not a signal.

### Persistent, or one bad month?

![Monthly PRR and case count for clozapine and neutropenia across 24 months, present in all 24 months with 5,571 cases in total](docs/assets/dashboard_3_trend.png)

The same pair month by month. Present in all 24 months, 5,571 cases in total, with PRR
holding well above 1 throughout. A single-month spike would look nothing like this.
