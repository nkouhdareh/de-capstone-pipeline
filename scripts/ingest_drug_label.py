"""
Ingest openFDA drug/label (SPL) into the bronze layer, using the bulk download files.

Why bulk files (not the paged API like drug/event):
- label has 261,258 records — far past the API's 26,000 paging limit.
- The 14 bulk files get every record reliably in one pass, and need no API key.

Saves raw NDJSON to DATA_DIR/bronze/drug_label/part-XXXX.json (one record per line).

Run from the repo folder:
    python scripts/ingest_drug_label.py

Note: label records include full label TEXT, so this is large (several GB).
For a quick sample, set MAX_FILES = 1 below.
"""
...
ENDPOINT = "label"                       # openFDA drug endpoint
